...
class J3TunesAudioHandler extends BaseAudioHandler {
  ...
  static const int _queueLookahead = 3; // Look ahead to preload 3 songs
  ...
  Future<void> _playFromQueue(int index) async {
    try {
      ...
      if (success) {
        ...
        // Preloading is now handled in _setAudioSourceAndPlay
      } else {
        _handlePlaybackError();
      }
    } catch (e, stackTrace) {
  ...
  // Preload upcoming songs in the queue to ensure smooth transitions
    ...
    if (isOffline) {
      ...
      if (cachedUrl != null && cachedUrl is String && cachedUrl.isNotEmpty) {
        if (Uri.tryParse(cachedUrl)?.hasQuery ?? false) {
          ...
          // unawaited(updateRecentlyPlayed(songId)); // Moved to _setAudioSourceAndPlay
          return cachedUrl;
        }
      }
      ...
      try {
        ...
        logger.log('musify.getSong fallback URL for $songId: $fallbackUrl',
            null, null);
        if (fallbackUrl != null && fallbackUrl.isNotEmpty) { // Cache fallback URL
        ...
        if (onlineUrl != null && onlineUrl.isNotEmpty) {
          ...
          if (onlineSource != null) {
            try {
              ...
              // Start preloading after successful online fallback.
              Future.delayed(const Duration(seconds: 2), _preloadUpcomingSongs);
